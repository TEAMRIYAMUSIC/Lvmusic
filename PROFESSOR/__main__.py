...
async def init():
    ...
    for all_module in ALL_MODULES:
        importlib.import_module("GOOD. FATHER.plugins" + all_module)
    LOGGER("GOOD.FATHER.plugins").info("✦ 𝐀𝐥𝐥 𝐅𝐞𝐚𝐭𝐮𝐫𝐞𝐬 𝐋𝐨𝐚𝐝𝐞𝐝 𝐁𝐚𝐛𝐲...♥︎")
    await userbot.start()
    await PROF.start()
    try:
        await PROF.stream_call("https://te.legra.ph/file/29f784eb49d230ab62e9e.mp4")
    except NoActiveGroupCall:
        LOGGER("GOODFATHER").error(
    ...
    LOGGER("GOODFATHER").info(
        "✦ 𝗠𝗔𝗗𝗘 𝗕𝗬 𝐆𝐎𝐎𝐃 𝐅𝐀𝐓𝐇𝐄𝐑 ...♥︎"
    )
    await idle()
    await app.stop()
    await userbot.stop()
    LOGGER("GOODFATHER").info("✦ 𝗦𝗧𝗢𝗣 𝗠𝗨𝗦𝗜𝗖 𝗕𝗢𝗧...♥︎")
